...
async function analyzeProgram(): Promise<any> {
  const structure: any = {
    instructions: [],
    accounts: [],
  ...
  try {
    ...
    for (const file of rustFiles) {
      ...
      if (instructionMatches) {
        structure.instructions.push(...instructionMatches.map(match =>
          match.replace('pub fn ', '').replace('(', '')
        ));
      }
      ...
      if (accountMatches) {
        structure.accounts.push(...accountMatches.map(match =>
          match.replace('#[account]\n    pub struct ', '')
        ));
      }
    }
...
async function generateTestSuite(structure: any): Promise<void> {
  const testsDir = join('tests');
  const testFile = join(testsDir, 'forge-generated.ts');
...
describe("${getProgramName()} - FORGE Generated Tests", () => {
  ...
  ${structure.instructions.map((instruction: string) => `
  ...
  describe("PDA Derivation", () => {
    ${structure.pdas.map((pda: string, index: number) => `
...
async function installTestDeps(): Promise<void> {
  try {
...
function getDefaultParams(instruction: string): string {
  // Generate sensible default parameters based on instruction name
  if (instruction.includes('transfer')) return 'new anchor.BN(1000)';
  if (instruction.includes('mint')) return 'new anchor.BN(1000000)';
  if (instruction.includes('burn')) return 'new anchor.BN(1000)';
  if (instruction.includes('initialize')) return '';
  return '/* TODO: Add parameters */';
}

function getDefaultAccounts(instruction: string, structure: any): string {
  let accounts = '';
