...
async function upgradeCommand(targetVersion) {
...
function getCurrentAnchorVersion() {
    try {
...
function getLatestAnchorVersion() {
    try {
        const output = (0, child_process_1.execSync)('anchor --version', { encoding: 'utf8' });
...
function analyzeUpgrade(current, target) {
...
function displayUpgradePlan(info) {
...
async function performUpgrade(info) {
    console.log('🔧 Performing upgrade...\n');
    // Update Anchor.toml
    let anchorToml = (0, fs_1.readFileSync)('Anchor.toml', 'utf8');
    anchorToml = anchorToml.replace(/anchor_version\s*=\s*"[^"]+"/, `anchor_version = "${info.targetVersion}"`);
    (0, fs_1.writeFileSync)('Anchor.toml', anchorToml);
    console.log('✅ Updated Anchor.toml');
    // Update Cargo.toml files
    const { glob } = require('glob');
    const cargoFiles = await glob('**/Cargo.toml');
    for (const file of cargoFiles) {
        let cargoToml = (0, fs_1.readFileSync)(file, 'utf8');
        const oldVersion = info.currentVersion;
        const newVersion = info.targetVersion;
        // Update anchor-lang
        cargoToml = cargoToml.replace(new RegExp(`anchor-lang\\s*=\\s*"${oldVersion.replace(/\./g, '\\.')}"`, 'g'), `anchor-lang = "${newVersion}"`);
        // Update anchor-spl
        cargoToml = cargoToml.replace(new RegExp(`anchor-spl\\s*=\\s*"${oldVersion.replace(/\./g, '\\.')}"`, 'g'), `anchor-spl = "${newVersion}"`);
        (0, fs_1.writeFileSync)(file, cargoToml);
        console.log(`✅ Updated ${file}`);
    }
    // Run cargo update
    console.log('\n📦 Updating dependencies...');
    try {
        (0, child_process_1.execSync)('cargo update', { stdio: 'inherit' });
    }
    catch (error) {
